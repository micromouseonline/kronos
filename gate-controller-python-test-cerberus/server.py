...
def append_to_log(text):
    ...
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%u")[:-2]
            ...
            if len(event_ledger) < 2:
                ...
                for ev in event_ledger:
                    if (now - ev['arrival_time']).total_seconds() > 3.0:
                        append_to_log(f"UNMATCHED  EVENT (TIMEOUT) Triggered By: {ev['id']} (Mode: {ev['mode']}) | TSF: {ev['tsf']}")
                    else:
                        retained.append(ev)
            ...
            for i in range(len(event_ledger)):
                ...
                for j in range(i + 1, len(event_ledger)):
                    ...
                    if tsf_gap <= MATCH_THRESHOLD_US:
                        if current_ev['id'] != next_ev['id']:
                            ...
                            append_to_log(f"SUCCESSFUL MATCH {status_signature}: TSF Gap: {tsf_gap:>3} µs ({gap_ms:.4f} ms)")
                ...
                if not match_found:
                    if (now - current_ev['arrival_time']).total_seconds() > 3.0:
                        append_to_log(f"UNMATCHED  EVENT Triggered By: {current_ev['id']} (Mode: {current_ev['mode']}) | TSF: {current_ev['tsf']}")
                        matched_indices.add(i)
                    else:
...
class TimingGateHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # Super fast registration loop - minimizes lock holding time
        with ledger_lock:
            if gate_id in system_registry:
                if system_registry[gate_id]['status'] == 'OFFLINE':
                    append_to_log(f"INFO: Node [{gate_id}] has re-established communication.")
                system_registry[gate_id]['last_seen'] = datetime.datetime.now()
            ...
            event_ledger.append({
                'id': gate_id, 'tsf': tsf_value, 'bssid': ap_bssid, 'mode': clock_mode, 'arrival_time': datetime.datetime.now()
            })
